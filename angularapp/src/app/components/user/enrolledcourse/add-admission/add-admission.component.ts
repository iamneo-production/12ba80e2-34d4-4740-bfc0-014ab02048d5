...
export class AddAdmissionComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.addStudentForm = new FormGroup({
      ...
      phonenumber: new FormControl(null, [Validators.required,Validators.pattern(/^(?!([\d])\1{9}$)\d{10}$/)]),
      alternatenumber: new FormControl(null, [Validators.required,Validators.pattern(/^(?!([\d])\1{9}$)\d{10}$/)]),
